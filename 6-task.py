...
def load_books(cursor, num_books=1000):
    ...
    books = [
        (fake.sentence(nb_words=4), random.choice(author_ids), fake.date_between(start_date='-10y', end_date='today'))
...
def load_borrowing_records(cursor, num_records=5000):
    ...
    records = [(random.choice(book_ids), random.choice(member_ids),
                fake.date_between(start_date='-2y', end_date='today'),
                None if random.random() < 0.5 else fake.date_between(start_date='today', end_date='today')) for _ in
               range(num_records)]
    cursor.executemany(
        "INSERT INTO BorrowingRecords (book_id, member_id, borrow_date, return_date) VALUES (%s, %s, %s, %s)", records)
